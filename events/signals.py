...
def send_rsvp_email_notification(sender, instance, action, reverse, model, pk_set, **kwargs):
    """
    ...
    if action == 'post_add': # User has been added to participants
        for user_pk in pk_set:
            ...
            html_message = render_to_string('emails/rsvp_confirmation.html', {'user': user, 'event': event})
            plain_message = strip_tags(html_message)
            from_email = 'noreply@yourdomain.com' # Replace with your actual email
            to_email = [user.email]

            send_mail(subject, plain_message, from_email, to_email, html_message=html_message)
            print(f"RSVP Confirmation email sent to {user.email} for event {event.name}") # For console output in development
